...
class Query():
    ...
    def make_or_update(self, session, model, record, primary_key, foreign_keys=None):
        ...
        if exists is None:
            querylogger.debug(
                f"{record[first_primary_key]} did not return existing row.  Generating placeholder object.")
            ...
            with session.no_autoflush:
                querylogger.debug(
                    f'Updating existing record for {record[first_primary_key]}')
                self.update_record(session=session, record=record,
    ...
    def get_known_constraints(model):
        ...
        foreign_keys = [list(keys)[0]
                        for keys in foreign_key_sets]  # TODO: build
        return {
    ...
    def validate_constraints(session, model, record, foreign_keys):
        for target_fullname in foreign_keys:
            ...
            exists = check_exists(
                session=session, model=key_model, record=record, id_column=key)
            if not exists:
                querylogger.info(
                    f"{record['key']} did not return existing row.  Generating placeholder object.")
